    ...
    while True:
        if len(wait_handler_msgs) != 0:
            ...
            if now == 1:
                ...
                subprocess.Popen(f"ffmpeg -re -loglevel level+info -nostats -stream_loop -1 -i tmp.mp3 -map 0:a:0 -acodec libopus -ab 128k -filter:a volume=0.8 -ac 2 -ar 48000 -f tee [select=a:f=rtp:ssrc=1357:payload_type=100]rtp://{ip}:{port}?rtcpport={rtcp_port}",shell=True)
                now = 5
            else:
